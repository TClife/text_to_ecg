...
- Sparse attention variants (axial, convolutional)
- DeepSpeed sparse attention integration
"""
...
def apply_pos_emb(pos_emb, qkv):
    """Apply rotary positional embedding to query, key, value."""
    n = qkv[0].shape[-2]
    pos_emb = pos_emb[..., :n, :]
...
class SparseConvCausalAttention(nn.Module):
    """Sparse attention with convolutional pattern.

    Attends to local window defined by kernel_size and dilation.
    """

    def __init__(self, dim, seq_len, image_size=32, kernel_size=5, dilation=1,
                 heads=8, dim_head=64, dropout=0., stable=False, **kwargs):
        super().__init__()
    ...
    def forward(self, x, mask=None, rotary_pos_emb=None):
        ...
        # Image attention with convolutional pattern
...
class SparseAxialCausalAttention(nn.Module):
    """Sparse axial attention (row or column)."""

    def __init__(self, dim, seq_len, image_size=32, axis=0, heads=8, dim_head=64,
                 dropout=0., stable=False, **kwargs):
        super().__init__()
...
class SparseAttention(Attention):
    ...
    def __init__(self, *args, block_size=16, text_seq_len=256,
                 num_random_blocks=None, **kwargs):
