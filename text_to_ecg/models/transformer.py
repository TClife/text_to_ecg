"""
Transformer backbone for autoregressive ECG generation.
"""
...
from text_to_ecg.models.attention import (
    Attention,
    SparseAttention,
...
class DivideMax(nn.Module):
    """Divide by maximum value for numerical stability."""
...
class LayerScale(nn.Module):
    """Layer scaling for deep transformers.

    From: https://arxiv.org/abs/2103.17239
    """
...
class PreShiftToken(nn.Module):
    """Token shifting for efficient local attention."""
...
class Transformer(nn.Module):
    """Transformer backbone for autoregressive generation.

    Args:
        dim: Model dimension
        depth: Number of layers
        seq_len: Maximum sequence length
        reversible: Use reversible layers for memory efficiency
        ...
        attn_types: Tuple of attention types to cycle through
        image_fmap_size: Image feature map size (for ECG: sequence length after VAE encoding)
        ...
        *,
        ...
        for ind, sparse_attn, attn_type in zip(range(depth), sparse_layer, attn_type_layer):
            if attn_type == 'full':
                ...
                attn_class = partial(SparseAxialCausalAttention, seq_len=seq_len, axis=0,
                                     image_size=image_fmap_size, stable=stable)
            elif attn_type == 'axial_col':
                attn_class = partial(SparseAxialCausalAttention, seq_len=seq_len, axis=1,
                                     image_size=image_fmap_size, stable=stable)
            elif attn_type == 'conv_like':
                attn_class = partial(SparseConvCausalAttention, seq_len=seq_len,
                                     image_size=image_fmap_size, stable=stable)
            else:
                raise ValueError(f'attention type "{attn_type}" is not valid')

            attn = attn_class(dim, causal=causal, seq_len=seq_len, heads=heads,
            ...
            if shift_tokens:
                attn, ff = map(
                    lambda t: PreShiftToken(t, image_size=image_fmap_size, seq_len=seq_len),
                    (attn, ff)
                )
